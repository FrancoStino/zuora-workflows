...
```mermaid
flowchart TD
    A[Trigger<br/>UI/CLI/Scheduler] --> B[Dispatch<br/>SyncCustomersJob]
    B --> C[Queue processes<br/>the job]
    C --> D[WorkflowSyncService]
    D --> E1[Authenticate with<br/>Zuora OAuth 2.0]
    D --> E2[Fetch workflows<br/>paginated]
    D --> E3[Download JSON export<br/>for each workflow]
    D --> E4[Save/update<br/>in database]
    D --> E5[Extract and<br/>synchronize tasks]
    E1 --> F[Workflows available<br/>in UI]
    E2 --> F
    E3 --> F
    E4 --> F
    E5 --> F
...
| Attempt | Expected after | Total elapsed |
|---------|---------------|---------------|
...
| Workflows | Estimated time | API calls |
|-----------|---------------|------------|
