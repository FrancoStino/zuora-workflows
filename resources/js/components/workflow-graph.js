...
export function initWorkflowGraph( containerId, workflowData ) {
	...
	// Get container dimensions and make paper responsive
	const updatePaperSize = () => {
		const rect = container.getBoundingClientRect();
		const width = rect.width || container.clientWidth;
		const height = Math.max(400, window.innerHeight * 0.6); // Minimum height 400px or 60% of viewport
		
		// Force paper to use container width
		paper.setDimensions(width, height);
		
		// Also set the container style to ensure it doesn't overflow
		container.style.width = '100%';
		container.style.maxWidth = '100%';
		container.style.overflow = 'hidden';
		
		// Get the paper element and force its dimensions
		const paperElement = paper.el;
		if (paperElement) {
			paperElement.style.width = '100%';
			paperElement.style.maxWidth = '100%';
			paperElement.style.minWidth = '100%';
			paperElement.style.overflow = 'hidden';
		}
		
		// Also force the SVG element to respect container bounds
	...
	const resizeObserver = new ResizeObserver(() => {
		updatePaperSize();
		// Re-center content after resize with a small delay to ensure DOM is updated
		setTimeout(() => {
			const containerWidth = container.clientWidth || container.getBoundingClientRect().width;
			paper.fitToContent({
				padding: 20, // Reduced from 50 to 20
				allowNewOrigin: 'any',
				minScale: 0.1,
				maxScale: 1.0
			});
			
			// Check if content is wider than container and scale if needed
			const contentBBox = paper.getContentBBox();
			if (contentBBox.width > containerWidth - 40) { // Reduced from 100 to 40
				const scale = (containerWidth - 40) / contentBBox.width;
				if (scale < 1.0) {
	...
	// Also handle initial content fitting after graph is built
	setTimeout(() => {
		const containerWidth = container.clientWidth || container.getBoundingClientRect().width;
		paper.fitToContent({
			padding: 20, // Reduced from 50 to 20
	...
	try {
		...
		// Center the paper content with constraints
		const containerWidth = container.clientWidth || container.getBoundingClientRect().width;
		paper.fitToContent( {
			padding: 20, // Reduced from 50 to 20
			allowNewOrigin: 'any',
			minScale: 0.1, // Allow scaling down if content is too wide
			maxScale: 1.0  // Don't scale up beyond actual size
		} );
		...
		addZoomAndPanControls( container, paper, resizeObserver );
...
function layoutGraph( graph ) {
	...
	g.setGraph( {
		...
		nodesep: 40,    // Reduced from 80 to 40
		edgesep: 20,    // Reduced from 80 to 20
		ranksep: 80,    // Reduced from 150 to 80
		marginx: 20,    // Reduced from 50 to 20
		marginy: 20     // Reduced from 50 to 20
	} );
...
function createNode( id, label, type, color ) {
	const node = new joint.shapes.standard.Rectangle( {
		id: id,
		size: {width: 140, height: 50}, // Reduced from 180x60 to 140x50
		attrs: {
			body: {
				fill: color,
				stroke: darkenColor( color, 20 ),
				strokeWidth: 2,
				rx: 6, // Reduced from 8 to 6
				ry: 6  // Reduced from 8 to 6
			},
			label: {
				text: truncateText( label, 15 ), // Reduced from 20 to 15
				fill: '#ffffff',
...
function addZoomAndPanControls( container, paper, resizeObserver ) {
